  ...
  for (y = 1; y < height - 1; ++y) {
    ...
    while (indX < width - 1) {
      ...
      if (updating_mask[index] == COLOR_BACKGROUND) {
        ...
        uint8_t value_neighbor=image_data[index_neighbor];
        ...
        if (position[shift] < NUMBER_OF_HISTORY_IMAGES) {
          ...
          if(updating_mask[index_neighbor]!=COLOR_CURRENTOBJ)
                historyImage[index_neighbor + position[shift] * width * height] = value_neighbor;
        }
        else {
          int pos = position[shift] - NUMBER_OF_HISTORY_IMAGES;
          historyBuffer[index * numberOfTests + pos] = value;
          if(updating_mask[index_neighbor]!=COLOR_CURRENTOBJ)
                historyBuffer[index_neighbor * numberOfTests + pos] = value_neighbor;
        }
      }
